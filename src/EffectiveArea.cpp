...
{	
	H5::H5File file("/Users/marjon/Dropbox (MIT)/work/IceCube/decay/neutrino_decay/verosimilitud/data/effective_area.h5",H5F_ACC_RDONLY);
	...
	for (unsigned int year=0; year<2; year++)
	{
		...
		attr = new H5::Attribute(group->openAttribute("experimental_livetime(seconds)"));
		...
		for (unsigned int flavor=0; flavor<2; flavor++)
		{
			for (unsigned int anti=0; anti<2; anti++)
			{
				...
				std::string data_name=sl+year_names[year]+sl+nu+flavor_names[flavor]
					+anti_names[anti];
	
				H5::DataSet dataset = file.openDataSet((data_name+sl+s_area).c_str());
				...
				dataset.read(areas[year][flavor][anti]->GetDataPointer(), H5::PredType::NATIVE_DOUBLE,mspace1,filespace);

				//Now read in the bin edges.
				for (unsigned int edgedim=0; edgedim<3; edgedim++)
				{
	
					H5::DataSet dataset = file.openDataSet((data_name+sl+
						s_edges+s_dims[edgedim]).c_str());
					...
					dataset.read(edges[year][flavor][anti][edgedim]->GetDataPointer(), H5::PredType::NATIVE_DOUBLE,mspace1,filespace);
				}
			}
		}
	}
